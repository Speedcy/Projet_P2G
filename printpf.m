...
if OUT_BRANCH && (success || OUT_FORCE)
    ...
    fprintf(fd, '\n|     Branch Data                                                              |');
    fprintf(fd, '\n================================================================================');
    fprintf(fd, '\nBrnch   From   To    From Bus Injection   To Bus Injection     Loss (I^2 * Z)     Pertes     Courant');
    fprintf(fd, '\n  #     Bus    Bus    P (MW)   Q (MVAr)   P (MW)   Q (MVAr)   P (MW)   Q (MVAr)  Pourcents   Ampères');
    fprintf(fd, '\n-----  -----  -----  --------  --------  --------  --------  --------  --------  --------   --------');
    fprintf(fd, '\n%4d%7d%7d%10.2f%10.2f%10.2f%10.2f%10.3f%10.2f%13.2f%13.2f%13.2f', ...
            [   (1:nl)', branch(:, [F_BUS, T_BUS]), ...
                branch(:, [PF, QF]), branch(:, [PT, QT]), ...
                real(loss), imag(loss), ...
                (real(loss)./branch(:,PT))*100, ...
                ((real(loss)*10^6)./(branch(:,[BR_R,BR_B])*81)).^(0.5)/3 ...
                %((branch(:,PT)*10^(6))./(bus(:, [VM, VMAX])*90000))/(sqrt(3)), ...
